...
- [dsh-team-rooms](https://github.com/PerryLink/dsh-team-rooms) - 持久化、跨会话的多 Agent 团队房间：成员、消息总线、共享任务板、审批门控交接与共享时间线，重启后仍然保留。
...
- [dsh-kb-rag](https://github.com/Breeze136/dsh-kb-rag) - 本地文献知识库 RAG：10 个工具（PDF/文件夹/Zotero 入库、BM25+向量+重排混合检索、DOI 可点击溯源问答、范围/严格模式、去重/清空/统计、DOI 补库、后台任务进度），全本地 bge 嵌入 + 单文件 SQLite，实测 242 篇 86s 入库、2 万块亚秒热查询。
...
- [exoticknight/dsh-just-chat](https://github.com/exoticknight/dsh-just-chat) - 一键打开原生对话入口，每次创建独立工作区和会话。
...
- [泡泡猫 DSH 桌面版](https://github.com/zmm863-commits/dsh-desktop-pack) - Windows NSIS 安装包：内置 Node.js 与 Git，预装 17 个插件与泡泡猫玻璃主题，安装后双击桌面快捷方式启动 DSH Web UI。
...
- [xgone/dsh-netshell](https://github.com/xgone/dsh-netshell) - DeepSeek Harness Web 的本地与远程 SSH 终端，提供三级权限、基于规则的命令确认与 DSH 凭据存储。护栏仅覆盖经过本插件的命令，不提供安全沙箱。
...
- [sofagent](https://github.com/KongFangXun/sofagent) - 提交时 agent 治理 harness：24 条确定性规则审计 git diff（密钥泄漏、越权改动、盲目修改、注入痕迹），HMAC 链防篡改历史，DSH 插件家族（audit / gate / rollback / inject / ontology / evolve / commons / daemon / fde / harness）经 SkillHub 分发。
...
- [dsh-im](https://github.com/xmanrui/dsh-im) - 将飞书、微信、钉钉、Slack、Telegram、Discord 等平台的聊天机器人接入 DeepSeek Harness，支持按机器人独立绑定工作区、模型和会话。
...
- [Nicholaskin/vision-exp-tile](https://github.com/Nicholaskin/vision-exp-tile) - 面向视觉大模型的大图智能识图插件：800×800 无损切块识别（smart/pipeline/full 三策略）、本地 OCR（前处理+手写分流）、可选多厂商 GPU 加速（DirectML 覆盖 NVIDIA/AMD/Intel，CUDA/OpenVINO），自动回退 CPU。MIT 开源。
