...
- [dsh-vision-proxy](https://github.com/Flyvhidbwo/dsh-vision-proxy) - DeepSeek 大脑 + 自动识图：GUI 附加的每张图片自动经任意 OpenAI 兼容 VLM 转译为文字后交给纯文本 DeepSeek——无 key 默认免费匿名端点（零配置），有 key 自动走百炼快速通道（百炼/智谱/Ollama/OpenRouter 等均支持）
