...
- [dsh-statecore](https://github.com/yul761/dsh-statecore) - Auditable project memory powered by StateCore: auto-ingests every user/assistant message and auto-injects a budgeted memory digest on each pre-step with no model cooperation, re-injects after compaction so `/compact` never forgets; five native tools (remember/recall/facts/why/forget) where `why` returns a fact's evidence and full supersession chain and `forget` retires instead of deleting; digest runs on the host's own `ctx.llm`, keyless embedded SQLite, a Settings page, and the same store is readable from any MCP client via `statecore-mcp`.
...
- [dsh-paperdesk](https://github.com/shiyan688/dsh-paperdesk) - Paper library and reading workbench: arXiv search, a local plain-file library (metadata / PDF / extracted full text under one root), and three-level reading notes (overview / understanding / critique) kept as markdown next to the PDFs; seven model-facing tools plus a web panel. No build step, zero runtime dependencies, loopback-only HTTP API. npm: `dsh-paperdesk`.
## Tools & Utilities
