...
- [dsh-skills-anywhere](https://github.com/noteflowai/dsh-skills-anywhere) - Live skill provider on `ctx.skills`: adds skills from 60+ other agents' directories (Claude Code, Codex, Cursor, Gemini CLI, Copilot), Claude Code plugin marketplaces, and any git skill repository to the dsh catalog without copying or symlinking; dedupes, repairs frontmatter, resolves name collisions; CLI (list, add, sync, doctor).
